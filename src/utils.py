##########################################################
###   This file is a copyright of python3 function     ###
###   --------------------------------------------     ###
###   Utilities (Head-pose Detection)                  ###
###   Before Modified by Qhan                          ###
###   Last Modified by Petros Apostolou                ###
...
class Annotator():
    ...
    def __init__(self, im, angles=None, bbox=None, lm=None, rvec=None,
            tvec=None, cm=None, dc=None, b=10.0):
        self.im = im
    ...
    def draw_bbox(self):
        ...
        cv2.rectangle(self.im, (x1, y1), (x2, y2), Color.yellow, self.ls)
    ...
    def draw_info(self, fontColor=Color.green):
        x, y, z = self.angles
        px, py, dy = int(5 * self.fs), int(25 * self.fs), int(30 * self.fs)
        font = cv2.FONT_HERSHEY_DUPLEX
        fs = self.fs
        #cv2.putText(self.im, "rot(x): %+06.2f" % x, (px,4*py), font,
        #        fontScale=fs, color=fontColor, thickness=2)
        #cv2.putText(self.im, "rot(y): %+06.2f" % y, (px, (4*py + dy)), font,
        #        fontScale=fs, color=fontColor, thickness=2)
        #cv2.putText(self.im, "rot(z): %+06.2f" % z, (px, (4*py + 2*dy)), font,
        #        fontScale=fs, color=fontColor, thickness=2)
        #if x >= 12.5:
        #    cv2.putText(self.im, "TIC-DETECTED [ x(rot) = %+06.2f]" %x, (20,200), font, fontScale=fs, color=Color.red, thickness=2)
        #    time.sleep(0.25)
